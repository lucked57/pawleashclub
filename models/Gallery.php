<?php
    ...
    public function upload()
    {
        if ($this->validate()) { 
            foreach ($this->imageFiles as $file) {
               ...
               if(filesize($copyfile) > 190000) {
                ...
                //$model->id_albom = $id_albom;
                $model->id_albom = Uservaludate::validateInput($this->id_albom);
       ...
       public function delete_img($id){
        ...
        public function delete_albom($id){
            
            $table = Gallery_paw::find()->asArray()->where(['id_albom' => $id])->all();

                foreach ($table as $table_del) {

                    if(!empty($table_del['img_full'])){
                    unlink('./'.$table_del['img_full']);
                    }
                    if(!empty($table_del['img_min'])){
                        unlink('./'.$table_del['img_min']);
                    }
                    ...
                    $table = Gallery_paw::findone($table_del['id']);
